...
.filter('highlight', ['$sce', function ($sce) {
  return function (input, searchParam) {
    ...
    if (searchParam) {
      var searchParamFixed = fixSearch(searchParam); // escape parenthesis and other regex operators
      var words = '(' +
            searchParamFixed.split(/\ /).join(' |') + '|' +
            searchParamFixed.split(/\ /).join('|') +
          ')';
...
// this function escapes any terms in the searchTerm that would be part of the operators of a regex
var fixSearch = function(searchTerm) {
  var toReturn = searchTerm.replace('(', '\\('); // replace parenthesis
  toReturn = toReturn.replace(')', '\\)');
  // here add escapes for any other characters that could appear in a university name that are regex operators
  return toReturn;
};
